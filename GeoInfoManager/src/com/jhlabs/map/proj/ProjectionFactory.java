...
//		projection.setProjectionLatitude2Degrees( 0 );
		...
		if ( projection instanceof TransverseMercatorProjection ) {
            s = params.get( "zone" );
            if ( s != null ) {
                ((TransverseMercatorProjection)projection).setUTMZone( Integer.parseInt( s ), issouth);
            }
        }
...
//pm
	...
	static Hashtable<String, Class> registry;
...
//		register( "wink2", Projection.class, "Winkel II" );
	...
	public static Projection getNamedPROJ4CoordinateSystem( String name ) {
		String[] files = {"nad83","epsg_austnz","epsg_china","epsg_na","epsg_eu","epsg_sa"};
		try {
            int p = name.indexOf(':');
            if ( p >= 0 )
                return readProjectionFile( name.substring(0,p), name.substring(p+1) );
            String test = (String) name.subSequence(0, 2);
            int usefile = 0;
            if(test.contentEquals("au"))
            	usefile = 1;
            else if(test.contentEquals("ch"))
            	usefile = 2;
            else if(test.contentEquals("na"))
            	usefile = 3;
            else if(test.contentEquals("eu"))
            	usefile = 4;
            else if(test.contentEquals("sa"))
            	usefile = 5;
            else
            	usefile = 0;
			Projection projection = readProjectionFile( files[usefile], name );
			if ( projection != null )
				return projection;
		}
		catch ( IOException e ) {
