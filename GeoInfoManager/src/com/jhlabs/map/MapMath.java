...
public class MapMath {
	 ...
	 * @return
	 */
	public static int getUtmZone(double lng){
		int zone = 1;
		int zonebase = 0;
		if(lng < 0)
			lng = 180.0 + lng;
		...
		zone = (int)Math.floor(lng/6.0);
		zone = zonebase + zone+1;
	 ...
	 * @return
	 */
	public static Point2D.Double MeterstoFeet(boolean isfeet, Point2D.Double pt){
		if(isfeet){
			pt.x = pt.x * 3.280833333333333333;
			pt.y = pt.y * 3.280833333333333333;
		}
		return pt;
	}
...
/*	public static void latLongToXYZ(Point2D.Double ll, Point3D xyz) {
    ...
    public static double areatriangle(double a, double b, double c){
    	//abc are sides of triangle in meters
    	//s is semi_perimeter
    	double area = 0.0;
    	double s = (a+b+c)/2;
